...
function createServer(env: ConnectorEnv) {
  ...
  server.registerTool("get_location", {
    ...
    inputSchema: z.object({ location: z.string(), readMask: z.string().default("name,title,storeCode,websiteUri,phoneNumbers,categories,regularHours,specialHours,serviceArea,metadata,profile") }), annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: true },
  ...
  const parentSchema = z.object({ account: z.string(), location: z.string(), pageSize: z.number().int().min(1).max(50).default(50), pageToken: z.string().optional() });
  server.registerTool("list_reviews", {
    title: "List GBP reviews", description: "Lists reviews for one Business Profile location.", inputSchema: parentSchema,
    annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: true },
  }, async ({ account, location, pageSize, pageToken }) => { try { const p = props(READ_SCOPE); const token = await refreshAccessToken(env, p.refreshToken); const parent = `${resourceName(account, "accounts")}/${resourceName(location, "locations")}`; const url = new URL(`https://mybusiness.googleapis.com/v4/${parent}/reviews`); url.searchParams.set("pageSize", String(pageSize)); if (pageToken) url.searchParams.set("pageToken", pageToken); return ok(await googleRequest(token, url.toString())); } catch (e) { return failed(e); } });

  server.registerTool("list_posts", {
    title: "List GBP posts", description: "Lists local posts for one Business Profile location.", inputSchema: parentSchema,
    annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: true },
