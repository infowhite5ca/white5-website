...
async function uploadAttachments(accessToken, accountId, files) {
  ...
  for (const [index, file] of files.entries()) {
    const fileName = clean(file.name, 120) || `project-photo-${index + 1}.jpg`;
    multipart.append("attach", file, fileName);
  }

  const url = `${ZOHO_MAIL_API}/accounts/${encodeURIComponent(accountId)}/messages/attachments?uploadType=multipart&isInline=false`;
  ...
  if (!response.ok || uploaded.length !== files.length) {
    const description = clean(result?.status?.description || result?.data?.moreInfo || "", 500);
    throw new Error(`Zoho attachment upload failed: ${description || response.status}`);
  }

  const attachments = uploaded.map((item) => ({
    storeName: item?.storeName,
    attachmentPath: item?.attachmentPath,
    attachmentName: item?.attachmentName,
  }));

  if (attachments.some((item) => !item.storeName || !item.attachmentPath || !item.attachmentName)) {
    throw new Error("Zoho attachment upload failed: incomplete attachment response");
  }
...
export async function handleDeckFenceQuoteV5(request, env) {
...
export function handleDeckFenceConfigV5(request, env) {
