...
async function uploadAttachment(accessToken, accountId, file, fallbackName) {
...
async function sendLead(accessToken, account, fields, files) {
  ...
  for (const [index, file] of files.entries()) {
    attachments.push(await uploadAttachment(
      accessToken,
      account.accountId,
      file,
...
export async function handleDeckFenceQuoteV2(request, env) {
  ...
  if (clean(formData.get("company"), 100)) return json({ ok: true });
...
export function handleDeckFenceConfigV2(request, env) {
